...
const EXPORT_MAPPING: Record<string, ParamExport> = {
  ...
  // Added alongside their `GET /alumni` params so an export of a view using one
  // narrows the same way instead of quietly widening (#366/#592).
  country: { as: "multi", field: "country" },
  region: { as: "multi", field: "region" },
  past_title: { as: "multi", field: "past_title" },
  university: { as: "multi", field: "university" },
  degree: { as: "multi", field: "degree" },
  major: { as: "multi", field: "major" },
  worked_in_year: { as: "int", field: "worked_in_year" },
