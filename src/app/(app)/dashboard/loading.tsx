 ...
 * at each breakpoint, the bounded flex column, the pulled-up <main>, the 5:7
 * split. A
...
export default function Loading() {
  return (
    <main
      className={`flex-1 overflow-auto lg:flex lg:min-h-0 lg:flex-col lg:[scrollbar-gutter:auto] ${HERO_OVERLAP_CLASS}`}
    >
      <div className="px-4 pb-4 md:px-6 md:pb-6 lg:flex lg:flex-1 lg:flex-col">
        <PageSpinner className="md:hidden" />
        <div className="hidden flex-col gap-4 pt-4 md:flex md:pt-6 lg:flex-1 lg:gap-4 lg:pt-0">
          {/* FOUR tiles, 2x2 at tablet and 4-up from lg, at the top of a
              <main> pulled up by the same overlap the live page uses, so they
              straddle the shell's photo rather than starting below it. `count` and the column classes have
          ...
          {/* The row that absorbs the leftover height, like the live one. The
              panels take their height FROM it rather than declaring a min at
              `lg` — a `min-h-[26rem]` there would put the skeleton under a fold
              the live page does not have on a window that fits. */}
