...
export function SurveyCampaignConsole({
  ...
  return (
    <>
      ...
      <section className="overflow-hidden rounded-xl border border-navy-800 bg-navy-800 text-white shadow-sm">
        <div className="flex flex-col gap-5 p-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 max-w-xl text-sm text-white/80">
                Alumni get re-surveyed every two years — anyone never surveyed,
                or whose last completed survey is more than two years old. Grab
                the due list into a send batch, then send the verify-your-info
                request.
              </p>
            </div>
          </div>
          ...
          <div className="flex shrink-0 items-center gap-2.5">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2.5 text-sm font-semibold text-navy-800 transition hover:bg-brand-blue-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-white disabled:cursor-not-allowed disabled:bg-white/40 disabled:text-navy-800/50"
            >
            ...
            <button
              ...
              className="inline-flex cursor-not-allowed items-center gap-2 rounded-lg border border-white/30 px-4 py-2.5 text-sm font-semibold text-white/60"
            >
      ...
      {batch && (
        <section className="mt-4 rounded-xl border border-brand-blue-300 bg-brand-blue-50/60">
          <header className="flex flex-wrap items-center justify-between gap-3 border-b border-brand-blue-300 px-5 py-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  ...
                  {dueCount > batch.length ? (
                    <>
                      {" "}
                      Showing this page&apos;s {batch.length} of {dueCount} due
                      — narrow with filters, then re-grab to adjust who&apos;s
                      included.
                    </>
                  ) : null}
                </p>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1.5 rounded-lg bg-warning-50 px-3 py-1.5 text-xs font-semibold text-warning-600">
          ...
          <div className="flex flex-wrap gap-2 p-5">
            {batch.map((a) => (
              <Link
                ...
                className="inline-flex max-w-full items-center gap-2 rounded-full border border-gray-300 bg-white py-1 pl-1 pr-3 text-sm text-gray-900 hover:border-brand-blue-300 hover:bg-brand-blue-50/40"
              >
      ...
      <div className="mt-4 hidden overflow-hidden rounded-xl border border-gray-300 bg-white md:block">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-300 bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
              ...
              <th className="w-28 px-4 py-3">Grad year</th>
              <th className="px-4 py-3">Current employer</th>
              <th className="w-40 px-4 py-3">Survey status</th>
            </tr>
          </thead>
          <tbody>
            {items.map((a) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    {inBatch ? (
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-brand-blue-50 px-2.5 py-1 text-xs font-semibold text-navy-800">
                        <PackageCheck
                      ...
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-warning-50 px-2.5 py-1 text-xs font-semibold text-warning-600">
                        Due for re-survey
                      </span>
                    )}
                  </td>
