...
/**
 * DropdownMenu — accessible action menu (Radix). Used for per-row "⋯" action
 * menus (view/edit, add interaction, archive) and other contextual menus.
 */
...
>(({ className, destructive, ...props }, ref) => (
  <DropdownMenuPrimitive.Item
    ref={ref}
    className={cn(
      "flex cursor-pointer select-none items-center gap-2 rounded-md px-2.5 py-1.5 text-sm outline-none transition-colors [&_svg]:size-4 [&_svg]:shrink-0",
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    ref={ref}
    className={cn(
      "px-2.5 py-1.5 text-xs font-semibold uppercase tracking-wide text-gray-400",
      className,
    )}
    {...props}
  />
